...
class RIMCell(nn.Module):
	# One layr of RIM modules
	def __init__(self, 
		device, input_size, hidden_size, num_units, k, rnn_cell, enable_comm, input_key_size = 64, input_value_size = 400, input_query_size = 64,
	...
	def input_attention_mask(self, x, h, return_scores=False):
		...
		# Not-null values are selected BEFORE softmax
		# this results to activated experts with 0 attentio weights
		not_null_scores = attention_scores[:,:, 0]

		topk1 = torch.topk(not_null_scores,self.k,  dim = 1)
  
		# print(f'Not null sttention scores: {not_null_scores}')
		# print(f'Active experts for batch (mask) {topk1.indices.size(0)}: {mask_[0]}')
		...
		# print(f'Attention scores pre softmax: {attention_scores}')

		attention_probs = self.input_dropout(nn.Softmax(dim = -1)(attention_scores))

		# print(f'Attention scores post softmax: {attention_probs}')
  
		inputs = torch.matmul(attention_probs, value_layer) * mask_.unsqueeze(2)
  
		# print(f'Attention * mask: {attention_probs * mask_.unsqueeze(2)}')

		if return_scores:
			return inputs, mask_, attention_probs  #* mask_.unsqueeze(2)
		return inputs, mask_
